...
MEMORY_MAP = {
    '057': {'type': 'total_distance_m', 'size': 'double', 'base': 16},
    ...
    '1A1': {'type': 'mps?', 'size': 'double', 'base': 16},
...
def build_event(event_type: str, value=None, raw=None):
    return Event(event_type=event_type, value=value, raw=raw, timestamp=int(round(time.time() * 1000)))
...
def read_reply(cmd):
    ...
    if memory:
        size = memory['size']
        value_fn = SIZE_PARSE_MAP.get(size, lambda x: None)
        value = value_fn(cmd)
        if value is None:
...
def event_from(line):
    try:
        ...
        if cmd == STROKE_START_RESPONSE:  # with is "SS" from the waterrower
            return build_event(event_type='stroke_start',
            ...
            return build_event(event_type='stroke_end',
            ...
            return None
        elif cmd[:2] == MODEL_INFORMATION_RESPONSE:  # If MODEL information has been request, the model response would be "IV"
            return build_event(event_type='model', raw=cmd)  # Call the methode to create a dict with the model and the row command used for it "SE"
        ...
        elif cmd[:4] == PING_RESPONSE:  # if Ping response is received which is all the time the rower is in standstill
            return build_event(event_type='ping', raw=cmd)  # do nothing
        elif cmd[:1] == PULSE_COUNT_RESPONSE:  # Pluse count, count the amount of 25 teeth passed 25teeth passed = P1
            return build_event(event_type='pulse', raw=cmd)  # do nothing
        elif cmd == ERROR_RESPONSE:  # If Waterrower response with an error
            return build_event(event_type='error', raw=cmd)  # crate an event with the dict entry error and the raw command
        elif cmd[:2] == STROKE_START_RESPONSE:  # Pulse count, count the amount of 25 teeth passed 25teeth passed = P1
            print(cmd)
        else:
            return None
...
class Rower(IRower):
    def __init__(self):
        self._callbacks: Set[Callable[[Event], None]] = set()
    ...
    def register_callback(self, cb: Callable[[Event], None]):
        self._callbacks.add(cb)

    def remove_callback(self, cb: Callable[[Event], None]):
        self._callbacks.remove(cb)

    def notify_callbacks(self, event: Event):
